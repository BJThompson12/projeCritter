...
const Project = () => {
  ...
  if (loading1) {
    ...
    const backlogColumn = moodValues.filter((t) => t === 1);
    console.log(backlogColumn);
    const readyColumn = moodValues.filter((t) => t === 2);
    console.log(readyColumn);
    const inProgressColumn = moodValues.filter((t) => t === 3);
    console.log(inProgressColumn);

    const backlogSum = backlogColumn.reduce((sum, val) => sum + val);
    console.log('backlog sum here', backlogSum);
    const backlogMultiplier = backlogSum * 0.5;
    console.log('multiplier here', backlogMultiplier);

    readyColumn.push(0);
    console.log('ready column push', readyColumn);
    const readySum = readyColumn.reduce((sum, val) => sum + val);

    inProgressColumn.push(0);
    console.log('ready column push', inProgressColumn);
    const inProgressSum = inProgressColumn.reduce((sum, val) => sum + val);

    console.log('inProgress sum', inProgressSum);
